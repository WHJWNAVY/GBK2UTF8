...
int32_t cp936_mbtowc(conv_t conv, ucs4_t *pwc, const uint8_t *s, size_t n) {
    /* Try GBK first. */
    {
        int32_t ret = ces_gbk_mbtowc(conv, pwc, s, n);
        if (ret != RET_ILSEQ)
            return ret;
    }
...
int32_t cp936_wctomb(conv_t conv, uint8_t *r, ucs4_t wc, size_t n) {
    /* Try GBK first. */
    {
        int32_t ret = ces_gbk_wctomb(conv, r, wc, n);
        if (ret != RET_ILUNI)
            return ret;
    }
    /* Then handle the additional mappings. */
