...
static const uint16_t cp936ext_2uni_pagea6[181 - 159] = {
    /* 0xa6 */
    0xfe35, 0xfe36, 0xfe39, 0xfe3a, 0xfe3f, 0xfe40, 0xfe3d, 0xfe3e, 0xfe41, 0xfe42, 0xfe43,
    0xfe44, 0xfffd, 0xfffd, 0xfe3b, 0xfe3c, 0xfe37, 0xfe38, 0xfe31, 0xfffd, 0xfe33, 0xfe34,
};
static const uint16_t cp936ext_2uni_pagea8[128 - 122] = {
    /* 0xa8 */
    0x0251, 0xfffd, 0x0144, 0x0148, 0xfffd, 0x0261,
};

static int32_t cp936ext_mbtowc(conv_t conv, ucs4_t *pwc, const uint8_t *s, size_t n) {
    uint8_t c1 = s[0];
    if ((c1 == 0xa6) || (c1 == 0xa8)) {
        if (n >= 2) {
            uint8_t c2 = s[1];
            if ((c2 >= 0x40 && c2 < 0x7f) || (c2 >= 0x80 && c2 < 0xff)) {
                uint32_t i = 190 * (c1 - 0x81) + (c2 - (c2 >= 0x80 ? 0x41 : 0x40));
                uint16_t wc = 0xfffd;
                if (i < 7410) {
                    if (i >= 7189 && i < 7211)
...
static const uint16_t cp936ext_page01[16] = {
...
static const uint16_t cp936ext_page02[24] = {
...
static const uint16_t cp936ext_pagefe[24] = {
...
static int32_t cp936ext_wctomb(conv_t conv, uint8_t *r, ucs4_t wc, size_t n) {
    if (n >= 2) {
        uint16_t c = 0;
