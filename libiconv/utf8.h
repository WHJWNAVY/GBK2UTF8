...
int32_t utf8_mbtowc(conv_t conv, ucs4_t *pwc, const uint8_t *s, size_t n) {
    uint8_t c = s[0];
    ...
    if (c < 0x80) {
        ...
        *pwc = ((ucs4_t)(c & 0x0f) << 12) | ((ucs4_t)(s[1] ^ 0x80) << 6) | (ucs4_t)(s[2] ^ 0x80);
        ...
        if (!((s[1] ^ 0x80) < 0x40 && (s[2] ^ 0x80) < 0x40 && (s[3] ^ 0x80) < 0x40 && (c >= 0xf1 || s[1] >= 0x90) &&
            ...
            return RET_ILSEQ;
        *pwc = ((ucs4_t)(c & 0x07) << 18) | ((ucs4_t)(s[1] ^ 0x80) << 12) | ((ucs4_t)(s[2] ^ 0x80) << 6) |
               (ucs4_t)(s[3] ^ 0x80);
        return 4;
    } else
        return RET_ILSEQ;
...
int32_t utf8_wctomb(conv_t conv, uint8_t *r, ucs4_t wc, size_t n) /* n == 0 is acceptable */
{
    int32_t count;
    if (wc < 0x80)
        count = 1;
